...
def run(test, params, env):
    ...
    def check_output():
        ...
        out_list = res.stdout.strip().splitlines()[1:]
        out_dict = dict(item.strip().split("=") for item in out_list)

        if out_dict["dirtyrate.calc_status"] != "2":
            test.fail("Calculating dirty rate should be completed after %s seconds" % period)
        if out_dict["dirtyrate.calc_period"] != period:
            test.fail("Calculating period is not the same with the setting period %s" % period)
        dirty_rate = out_dict["dirtyrate.megabytes_per_second"]
        if abs(int(dirty_rate)/int(ram_size) - 1) > 0.75:
            test.fail("Dirty rate calculated %s has a big difference with the ram size %s "
                      "loaded in guest " % (dirty_rate, ram_size))
    ...
    if not libvirt_version.version_compare(7, 3, 0):
        test.cancel("domdirtyrate-calc command is not supported before version libvirt-7.3.0 ")
